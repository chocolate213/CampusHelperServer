...
public class UserDaoImpl implements UserDao {
    ...
    public User signIn(User user) throws CampusHelperException {
        ...
        //2. 插入用户登陆信息
        String insertUserInfoSQL = "UPDATE campus_helper.user_auths SET login_ip = ? AND login_time = ? AND last_login_time = ? WHERE campus_helper.user_auths.user_id = ? AND identity_type = ?";

        Long lastLoginTime = null;

        if (TextUtils.isEmpty(String.valueOf(authResult.getLastLoginTime()))) {
        ...
        String updatedUser = "SELECT" +
                ...
                "FROM user u JOIN user_auths ua ON user.user_id = ua.user_id " +
